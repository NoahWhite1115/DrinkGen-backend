...
class DrinkGenerator():
    def __init__(self, ingredient_matrix = None, ingredient_list = None, 
        ...
        if ingredient_list is None:
            self.ingredient_list = loadList('ingredients.csv')

        if measures_list is None:
            self.measures_list = loadList('measures.csv')
    ...
    def generateDrink(self):
        ...
        last_generated = markovStep(self.normalized_starter_array)
        drink_data.append(last_generated)

        measure_data.append(markovStep(self.normalized_measure_matrix[last_generated]))
        ...
        while random.uniform(0, 1) < 1/(times_run * len_modifier):
            ...
            last_generated = markovStep(self.normalized_ing_matrix[last_generated])
            drink_data.append(last_generated)
    ...
    def lookupValues(self, drink_data, measure_data):
        ...
        for i,j in zip(drink_data, measure_data):
            data.append(
                {'ingredient': ingredient_list[i].strip("\n"), 
                 'measure': measures_list[j].strip("\n")})
        return data